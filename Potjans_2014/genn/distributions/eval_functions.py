...
def __load_spike_times(path, name, begin, end, npop):
    ...
    """
    filename=['23E.csv',  '23I.csv',  '4E.csv',  '4I.csv',  '5E.csv',  \
              '5I.csv',  '6E.csv',  '6I.csv']
    data = {}
    dtype = {'names': ('time_ms', 'sender'),
             'formats': ('f8', 'i4')}
    #print(node_ids)
    ...
    for i_pop in range(npop):
        fn = os.path.join(path, filename[i_pop])
        data = np.loadtxt(fn, skiprows=1, dtype=dtype, delimiter = ",")

        data = np.sort(data, order='time_ms')
        # begin and end are included if they exist
        low = np.searchsorted(data['time_ms'], v=begin, side='left')
        high = np.searchsorted(data['time_ms'], v=end, side='right')
        print('N: ', max(data['sender'])+1)
        spike_times = []
        for i_neur in range(max(data['sender'])+1):
            spike_times.append([])
        for row in data:
            sender = row[1]
            time = row[0]/1000.0
            spike_times[sender].append(time)
...
def __smooth_hist(data, xmin, xmax, nx):
